...
class OrderController extends \App\Http\Controllers\Controller
{
    ...
    public function store(Request $r): JsonResponse
    {
        ...
        $order = DB::transaction(function () use ($data) {
            ...
            foreach ($data['items'] as $row) {
                ...
                if ($item instanceof Product && $item->track_inventory && $item->stock_quantity < $qty) throw ValidationException::withMessages(['items' => 'Insufficient stock for ' . $item->name . '.']);
                ...
                if ($item instanceof Product && $item->track_inventory) $item->decrement('stock_quantity', $qty);
                $subtotal += $line;
                $tax += $lineTax;
            }
    ...
    {
        if ($order->invoice) throw ValidationException::withMessages(['order' => 'An invoice already exists for this order.']);
    ...
    public function addPayment(Request $r, Invoice $invoice): JsonResponse
    {
        ...
        if ((float)$data['amount'] > ((float)$invoice->grand_total - (float)$invoice->paid_total)) throw ValidationException::withMessages(['amount' => 'Payment exceeds the invoice balance.']);
