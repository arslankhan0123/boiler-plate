...
class CatalogController extends \App\Http\Controllers\Controller
{
    ...
    public function store(Request $r): JsonResponse
    {
        ...
        if ($r->hasFile('image')) $data['image'] = $r->file('image')->store('ecommerce/categories', 'public');
    ...
    public function storeProduct(Request $r): JsonResponse
    {
        ...
        if ($r->hasFile('feature_image')) $data['feature_image'] = $r->file('feature_image')->store('ecommerce/products', 'public');
        $data['slug'] = $data['slug'] ?? Str::slug($data['name']) . '-' . Str::lower(Str::random(6));
        $data['created_by'] = Auth::guard('api')->id();
