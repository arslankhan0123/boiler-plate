...
/**
 * Create payload is a bare array of master references: `[{ "lookup_id": 5 }, ...]`.
 * The service resolves each id against the central lookups master and snapshots it.
 */
class StoreCountriesRequest extends FormRequest
{
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            if ($this->all() === []) {
                $validator->errors()->add('0', 'Provide at least one country to add.');
            }
        });
    }

    /**
     * The selected master lookup ids.
     *
     * @return list<int>
     */
